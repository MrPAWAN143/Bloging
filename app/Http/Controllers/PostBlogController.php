...
class PostBlogController extends Controller
{
    ...
    //         'user_id' => 'nullable|exists:users,id' // Validate the user exists in users table
    //     ]);
    ...
    //     // Manually compare and update the fields
    ...
    //     // If author is provided, update the user_id (author)
    //     if (isset($validatedData['user_id']) && $post_blog->user_id != $validatedData['user_id']) {
    //         $post_blog->user_id = $validatedData['user_id'];
    //         $changes = true;
    //     }
    ...
    public function update(Request $request, PostBlog $post_blog)
{
    ...
    // Format the slug only if provided
    $slug = $validatedData['slug'] ?? $post_blog->slug;
    if ($slug) {
        $slug = strtolower(trim(preg_replace('/\s+/', '-', $slug), '-'));
    }
    ...
    // Manually compare and update fields if they have changed
    if ($post_blog->heading !== $validatedData['heading']) {
        $post_blog->heading = $validatedData['heading'];
        $changes = true;
    }

    if ($post_blog->slug !== $slug) {
        $post_blog->slug = $slug;
        $changes = true;
    }
    ...
    // Handle updating the author if the user changes
    $currentUserId = auth()->id();
    if ($post_blog->user_id !== $currentUserId) {
        $post_blog->user_id = $currentUserId;
        $changes = true;
    }
    ...
    if ($changes) {
        ...
        return redirect()->route('editposts.index', $post_blog->id)
            ->with('info', 'No changes were made to the post.');
    }
}
